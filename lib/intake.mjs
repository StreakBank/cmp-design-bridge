...
const DEFAULT_PALETTE_TOP_N = 24;  // CONFIG-overridable (recipe.paletteTopN)
...
async function tryOcrSidecar(referencePng, outPath, dpr) {
  ...
  try {
    ...
    // tesseract.js v6+: word boxes live only under blocks[].paragraphs[].lines[]
    // and `blocks` is null unless explicitly requested in the output options.
    const { data } = await worker.recognize(referencePng, {}, { text: true, blocks: true });
    ...
    // A leaked worker_threads.Worker pins the event loop — the CLI would hang
    // after a successful-looking intake. Terminate unconditionally.
    if (worker) { try { await worker.terminate(); } catch { /* already down */ } }
  }
}

/** Flatten tesseract.js block-tree output (v6/v7 `blocks[].paragraphs[].lines[].words[]`;
 *  legacy flat `words[]` also accepted) into one word list. Pure (exported for tests). */
export function flattenOcrWords(data) {
  const raw = Array.isArray(data.words)
    ? data.words
    : (data.blocks || []).flatMap((b) => (b.paragraphs || []).flatMap((par) => (par.lines || []).flatMap((l) => l.words || [])));
  return raw.map((w) => ({
    text: w.text,
...
export async function intake(cfg, stateId, opts = {}) {
  ...
  const paletteTopN = recipe.paletteTopN ?? DEFAULT_PALETTE_TOP_N;
  ...
  try {
    ...
    const px = await page.evaluate(async ({ b64, box, targetW, clip, censusCap }) => {
      const img = new Image();
      ...
      const top = [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, censusCap)
    ...
    }, { b64: sourceB64, box: opts.contentBox || null, targetW: targetWidthPx, clip: clipPx, censusCap: Math.max(64, paletteTopN) });
    ...
    if (recipe.tokensSource) {
      ...
      if (themeTranslation === 'none') {
        ...
        tokenPairing = px.paletteTop.slice(0, paletteTopN).map((c) => {
    ...
    const manifest = {
      ...
      ocr,
      palette: {
        topColors: px.paletteTop.slice(0, paletteTopN).map((c) => ({ hex: hex(c.rgb), coveragePct: c.coveragePct })),
        snapThreshold,
        themeTranslation,
