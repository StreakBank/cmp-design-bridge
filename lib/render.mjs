...
// …but an INTENTIONALLY text-less render (a loading shimmer, an icon-only state)
// is still a real render if it produced a non-trivial element subtree. The blank
// guard fails only when BOTH are absent (no text AND a near-empty mount), which is
// the genuine broken-mount signal. Keeps the fail-loud purpose without flagging
// legitimately text-free states.
const MIN_CONTENT_NODES = 4;
...
export async function renderFrame(cfg, stateId, opts = {}) {
  ...
  // Optional: clip a tall scrollable design to the subject's fixed device
  // viewport height (CSS px). The subject screenshot is a fixed-height capture
  // (e.g. a phone viewport); a scrollable screen's design renders its FULL
  // natural height. Without clipping, the two images differ in height and the
  // grade can't align them above the fold. clipHeight clips the design to the
  // viewport so the above-fold regions are directly comparable; the natural
  // (pre-clip) height is preserved in the manifest so the below-fold content is
  // still surfaced (verify routes it to the Tier-2 gate). A no-op for any design
  // that already fits within clipHeight.
  const clipHeightCss = recipe.clipHeight ?? null;
  ...
  try {
    ...
    const info = await page.evaluate(() => {
      ...
      return {
        ...
        descendantCount: child ? child.getElementsByTagName('*').length : 0,
        sample: child ? (child.innerText || '').replace(/\s+/g, ' ').slice(0, 120) : '',
      };
    });

    // clipHeight: clip a tall scrollable design to the subject viewport BEFORE
    // the screenshot so the captured PNG matches the subject's fixed height. The
    // natural height (info.h) is already measured above and preserved in the
    // manifest; only the rendered capture is clipped. No-op when the design fits.
    const clipped = clipHeightCss != null && info.h > clipHeightCss;
    if (clipped) {
      await page.evaluate((ch) => {
        const child = document.getElementById('root').firstElementChild;
        if (child) { child.style.maxHeight = ch + 'px'; child.style.overflow = 'hidden'; }
      }, clipHeightCss);
    ...
    if (info.textLen < MIN_TEXT_LEN && info.descendantCount < MIN_CONTENT_NODES) renderReasons.push('blank-render(no text + empty subtree)');
    ...
    const manifest = {
      ...
      naturalHeightCss: info.h,
      clipHeightCss,
      clipped,
      capturedHeightCss: clipped ? clipHeightCss : info.h,
