...
export function allowlistFor(cfg, stateId) {
  ...
  for (const e of entries) {
    if (e.states !== undefined && !Array.isArray(e.states)) {
      // Fail loud: a malformed scope (e.g. a bare string) must not silently
      // widen a per-state suppression to every state.
      throw new Error(`allowlist entry "${e.id ?? '(no id)'}": \`states\` must be an array of stateIds (got ${typeof e.states})`);
    }
  }
  return entries.filter((e) => e.states === undefined || e.states.includes(stateId));
}
...
async function buildMontage(cfg, referencePng, cmpPng, outPng, cellWidth, referenceCaption) {
  ...
  try {
    ...
    const missingCell = (label) => `<div style="width:${cellWidth}px;height:300px;display:flex;align-items:center;justify-content:center;color:#8aa;font:13px monospace;border:1px dashed #334">${label}</div>`;
    const designB64 = existsSync(referencePng) ? readFileSync(referencePng).toString('base64') : null;
    const referenceCell = designB64
      ? `<img src="data:image/png;base64,${designB64}" style="width:${cellWidth}px;display:block;border:1px solid #1c2430"/>`
      : missingCell('reference missing<br/>(gates report referenceExists=false)');
    ...
    const subjectCell = cmpB64
      ...
      : missingCell('no subject screenshot<br/>(backfill slot)');
    await page.setContent(`<body style="margin:0;background:#05080f;padding:16px;font:12px monospace;color:#8aa">
      <div style="display:flex;gap:16px;align-items:flex-start">
        <figure style="margin:0"><figcaption style="padding:4px 0;color:#5ad">REFERENCE — ${referenceCaption}</figcaption>${referenceCell}</figure>
