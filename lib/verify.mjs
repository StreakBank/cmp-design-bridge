...
export function printVerify({ packet, gatesPass }) {
  ...
  if (packet.belowFoldRisk) {
    const note = packet.designClipped
      ? `reference clipped to the subject viewport (natural ${packet.designNaturalHeightCss}px CSS taller than the capture)`
      : `reference ${packet.designHeightPx}px > subject ${packet.cmpHeightPx}px`;
    console.log(`  ⚠ below-fold risk: ${note} — content below the fold is invisible to the image grade; confirm presence-contract items via Tier-2.`);
  }
